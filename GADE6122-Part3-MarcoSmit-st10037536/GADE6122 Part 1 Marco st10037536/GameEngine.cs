...
namespace GADE6122_Part_1_Marco_st10037536
{
    internal class GameEngine
    {
        ...
        {
            mapClass = new Map(minWidth, maxWidth, minHeight, maxHeight, 4, 4, 1);
        }
        ...
        {
            MovementEnum direction;
            for (int i = 0; i < mapClass.Leader.Length; i++)
            {
                if (MapClass.Leader[i].IsDead())
                {
                    MapClass.MapProp[MapClass.Leader[i].Y, MapClass.Leader[i].X] = new EmptyTile(MapClass.Leader[i].X, MapClass.Leader[i].Y) { Type = Tile.TileType.EmptyTile };
                    return;
                }

                direction = mapClass.Leader[i].ReturnMove();
                if (mapClass.Leader[i].ReturnMove() == direction)
                {

                    switch (direction)
                    {
                        case MovementEnum.Up:
                            Item? item = MapClass.GetItemAtPoint(MapClass.Leader[i].X, MapClass.Leader[i].Y - 1);
                            if (item is Gold)
                            {
                                MapClass.Leader[i].GoldCount += ((Gold)item).AmountGold;
                            }
                            break;
                        case MovementEnum.Down:
                            Item? item2 = MapClass.GetItemAtPoint(MapClass.Leader[i].X, MapClass.Leader[i].Y + 1);
                            if (item2 is Gold)
                            {
                                MapClass.Leader[i].GoldCount += ((Gold)item2).AmountGold;
                            }
                            break;
                        case MovementEnum.Left:
                            Item? item3 = MapClass.GetItemAtPoint(MapClass.Leader[i].X - 1, MapClass.Leader[i].Y);
                            if (item3 is Gold)
                            {
                                MapClass.Leader[i].GoldCount += ((Gold)item3).AmountGold;
                            }
                            break;
                        case MovementEnum.Right:
                            Item? item4 = MapClass.GetItemAtPoint(MapClass.Leader[i].X + 1, MapClass.Leader[i].Y);
                            if (item4 is Gold)
                            {
                                MapClass.Leader[i].GoldCount += ((Gold)item4).AmountGold;
                            }
                            break;
                    }

                    mapClass.Leader[i].Move(direction);
                    switch (MapClass.Leader[i])
                    {
                        case Swamp_Creature:
                            MapClass.MapProp[MapClass.Leader[i].Y, MapClass.Leader[i].X] = new Swamp_Creature(MapClass.Leader[i].X, MapClass.Leader[i].Y) { Type = Tile.TileType.Enemy };
                            break;
                        case Mage:
                            MapClass.MapProp[MapClass.Leader[i].Y, MapClass.Leader[i].X] = new Mage(MapClass.Leader[i].X, MapClass.Leader[i].Y) { Type = Tile.TileType.Enemy };
                            break;
                    }
                    switch (direction)
                    {
                        case MovementEnum.Up:
                            MapClass.MapProp[MapClass.Leader[i].Y + 1, MapClass.Leader[i].X] = new EmptyTile(MapClass.Leader[i].Y + 1, MapClass.Leader[i].X) { Type = Tile.TileType.EmptyTile };
                            break;
                        case MovementEnum.Down:
                            MapClass.MapProp[MapClass.Leader[i].Y - 1, MapClass.Leader[i].X] = new EmptyTile(MapClass.Leader[i].Y - 1, MapClass.Leader[i].X) { Type = Tile.TileType.EmptyTile };
                            break;
                        case MovementEnum.Left:
                            MapClass.MapProp[MapClass.Leader[i].Y, MapClass.Leader[i].X + 1] = new EmptyTile(MapClass.Leader[i].Y, MapClass.Leader[i].X + 1) { Type = Tile.TileType.EmptyTile };
                            break;
                        case MovementEnum.Right:
                            MapClass.MapProp[MapClass.Leader[i].Y, MapClass.Leader[i].X - 1] = new EmptyTile(MapClass.Leader[i].Y, MapClass.Leader[i].X - 1) { Type = Tile.TileType.EmptyTile };
                            break;
